# SimpLang Dictionary Library

> **"Store and retrieve data with ease."**

SimpLang now supports dictionaries — associative data structures that map keys to values. Dictionaries are perfect for organizing related data, like storing user profiles, configuration settings, or any key-value pairs.
...
A dictionary in SimpLang is a collection of key-value pairs, where each key is unique and maps to a value. Dictionaries are defined using curly braces `{}` with the syntax `{key: value, ...}`.
...
Keys in SimpLang dictionaries can be:
...
Since SimpLang dictionaries use Python's `dict`, accessing a non-existent key raises an error. Use `get` style patterns with `stdlib.dict_get`:
...
SimpLang's standard library provides dictionary helpers under `stdlib.dict_*`:
...
Dictionaries are a powerful data structure for organizing related data. With SimpLang's dictionary support:
...
For more examples, check out `examples/dictionary.simp` in the SimpLang repository.
